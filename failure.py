...
class Failure:
    ...
    def columnBuckling(self, x):
        ...
        #print(f"buckling{out}")
        ribs = self.Wingbox.ribs
        longerRibs = np.diff(ribs)
        #ribsForOut = np.array(list(map(lambda i: max(longerRibs[ribs < i]), self.Forces.span)))
        #out = cforceboolean * (math.pi ** 2 * K * E * self.Stringer.strIxx) / \
        #      ((longerRibs ** 2 * self.Stringer.areaArr))
        print(longerRibs)
        a = 0
        max = 10000000000000
        for i in longerRibs:
            minIn = np.amin((math.pi ** 2 * K * E * self.Stringer.strIxx) / ((i ** 2 * self.Stringer.areaArr)))
            print(minIn)
            if minIn < max:
                a = (math.pi ** 2 * K * E * self.Stringer.strIxx) / ((i ** 2 * self.Stringer.areaArr))
        print(a)
        return a
    ...
    def skinBuckling(self, x):
        ...
        top = math.pi ** 2 * k_c(self.ab(x)[0]) * E / 12 / (1 - v ** 2) * self.tb(x)[
